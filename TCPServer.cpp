...
TCPServer &TCPServer::listen() {
    ...
    if (::setsockopt(this->socket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
        throw NetworkException("Error at `setsockopt`", errno);
    }
    socklen_t socklen = sizeof(struct sockaddr_in);

    if (::bind(this->socket, this->serverAddress->ai_addr, socklen) == -1) {
        throw NetworkException("Error at `bind`", errno);
    }

    if (::listen(this->socket, 1024) < 0) {
        throw NetworkException("Error at `listen`", errno);
    }
    FD_ZERO(&read_fs);
    ...
    while(true){
        ...
        if(FD_ISSET(this->socket, &read_fs)){
            ...
            if( fd < 0 ){
                throw NetworkException("Error at `accept`", errno);
            }
